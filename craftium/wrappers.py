...
class NueToEnuVoxelObs(Wrapper):
    ...
    def _apply_wrapper_single_env(self, info):
        # NUEC -> ENUC
        info["voxel_obs"] = info["voxel_obs"].transpose(2, 0, 1, 3) #yzx -> xzy
        # (3[eun],) -> (3[enu],)
        info["voxel_obs_center"] = info["voxel_obs_center"][[0, 2, 1]] #xzy -> xyz
        info["player_pos"] = info["player_pos"][[0, 2, 1]] #xzy -> xyz
        info["player_vel"] = info["player_vel"][[0, 2, 1]] #xzy -> xyz
        info["cam_pos"] = info["cam_pos"][[0, 2, 1]] #xzy -> xyz
        info["cam_dir"] = info["cam_dir"][[0, 2, 1]] #xzy -> xyz
    ...
    def _apply_wrapper_vectorized_env(self, info):
        # need to account for the batch dimension
