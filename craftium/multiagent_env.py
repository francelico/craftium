...
class MarlCraftiumEnv():
        ...
        if self.mt_server.proc is None:
            ...
            for i in range(self.num_agents):
                ...
                for _ in range(self.init_frames):
                    _observation, _voxobs, _pos, _vel, _pitch, _yaw, _reward, _term = self.mt_channs[i].receive()
                    self.mt_channs[i].send([0]*21, 0, 0)  # nop action

                # receive the new info from minetest
                observation, _voxobs, _pos, _vel, _pitch, _yaw, reward, _term = self.mt_channs[i].receive()
    ...
    def step_agent(self, action):
        ...
        # receive the new info from minetest
        observation, _voxobs, _pos, _vel, _pitch, _yaw, reward, termination = self.mt_channs[agent_id].receive()
        if not self.gray_scale_keepdim and not self.rgb_observations:
            observation = observation[:, :, 0]
