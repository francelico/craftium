...
def yaw_pitch_cam_pos_to_extrinsics_intrinsics(yaws, pitchs, poses, fovs):
    fovs = torch.deg2rad(torch.tensor(float(fovs))).cuda()
    intrinsics = utils3d.torch.intrinsics_from_fov_xy(fovs, fovs)
    yaws = torch.tensor(float(yaws)).cuda()
    pitchs = torch.tensor(float(pitchs)).cuda()
    look_at_poses = torch.tensor([
        torch.sin(yaws) * torch.cos(pitchs),
        torch.cos(yaws) * torch.cos(pitchs),
        torch.sin(pitchs),
    ]).cuda()
    ups = torch.stack([torch.tensor([0, 0, 1])] * len(poses)).float().cuda()
    extrinsics = utils3d.torch.extrinsics_look_at(poses, look_at_poses, ups)
    return extrinsics, intrinsics

    # for loop version: todo test equivalence
    # extrinsics = []
    # intrinsics = []
    # for yaw, pitch, pose, fov in zip(yaws, pitchs, poses, fovs):
